    ...
    UI.prototype = {
        ...
        getClassName: function (item) {
            ...
            if (item.date < 1) {
             ...
             // 在当前月份只让已至日期高亮
             if (dateNow < item.date && monthNow == this.month && item.year == yearNow ) {
                className = className + ' gray';
                
                // 后面的月份全部日期不高亮
            } else if (monthNow < this.month && item.year == yearNow) {
                className = className + ' gray';
                // 前面的月份的日期全部高亮
            } else if (item.date > lastDate &&item.year == yearNow) {
                className = className + ' gray';
            } else if(item.year > yearNow) {
                className = className + ' gray';
            } 
            // 年
